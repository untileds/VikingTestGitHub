...
public class EditorToolScript : MonoBehaviour {
	...
	private Dictionary<int,int> existingCharacterUnlock = new Dictionary<int, int>();

	public void onSubmitCharacterUnlock(){
		//Debug.Log( UIInput.current.value);
		if(UIInput.current.value!=""){
			characterUnlock =  int.Parse( UIInput.current.value);
			Debug.Log("unLockCharacter "+characterUnlock);
		}
		//UIInput.current.value=null;
	}

	public void onSubmitSongName(){
		//Debug.Log( UIInput.current.value);
		if(UIInput.current.value!=""){
			songName = UIInput.current.value;
	...
	public void onGenerate(){
		...
		if(data!=null){
			...
			for(int i=0;i<xmlDoc.SelectNodes("Stage/Level").Count;i++){
				...
				existingCharacterUnlock.Add(int.Parse(stageNode.Attributes["level"].Value),int.Parse(stageNode.Attributes["unlockCharacter"].Value));
			}
			...
			if(existingLevelList.Contains(level)){
				...
				existingCharacterUnlock.Remove(level);
				...
				existingSongNameList.Add(level,songName);
				existingCharacterUnlock.Add(level,characterUnlock);
...
//				existingSongNameList.Sort();
			...
			for(int i =0;i<existingLevelList.Count;i++){
				xmlString+="<Level level = "+'"'+existingLevelList[i]+'"'+ " enemyFrequency = "+'"'+existingEnemyFrequencyList[existingLevelList[i]]+
					...
					+'"'+existingSongNameList[existingLevelList[i]]+'"'+" unlockCharacter="+'"'+existingCharacterUnlock[existingLevelList[i]]+'"'+ "></Level>";
			}
			xmlString+="</Stage>";
		//	XmlDocument xmlDocEdit = new XmlDocument();
			...
			string xmlString ="<?xml version="+'"'+"1.0"+'"'+" encoding="+'"'+"utf-8"+'"'+"?> \n"+
				...
				+'"'+songName+'"'+" unlockCharacter="+'"'+characterUnlock+'"'+ "></Level>\n</Stage>";
			XmlDocument xmldoc = new XmlDocument();
			Debug.Log(xmlString);
			xmldoc.LoadXml(xmlString);
			xmldoc.Save(Application.dataPath +"/Resources/GameLevelData/GameLevel.xml");
